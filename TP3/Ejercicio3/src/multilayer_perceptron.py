...
class MultilayerPerceptron:
    ...
    def train(self):
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            O, caches = self.model_forward(self.input_data, parameters,
            ...
            gradients = self.model_backward(O, self.expected_data, caches, self.hidden_activation,
                                       self.output_activation)

            # Actualización de parámetros según el algoritmo de optimización seleccionado
            if input_handler.optimizer == 'MOMENTUM':
                parameters, vel =  self.__update_params_momentum(self, gradients, vel, input_handler.momentum_alpha)
            elif input_handler.optimizer == 'ADAM':
                t += 1
                parameters, M, V = self.__update_params_adam(self, gradients, M, V, t, input_handler.beta1,
                                                        input_handler.beta2, input_handler.epsilon)
            else:
                parameters =  self.__update_params_gd(self, gradients)
    ...
    def __model_forward(self):
        ...
        for l in range(1, L):  # l=0 is the input
            ...
            V, cache = self.excitation_activation_forward(self.weights['W' + str(l)], b, self.hidden_activation)
            caches.append(cache)
            # activate output layer
            O, cache = self.excitation_activation_forward(self.weights['W' + str(L)], self.weights['b' + str(L)],
                                                     self.output_activation)
        caches.append(cache)
        return O, caches
    ...
    def __model_backward(self, O, Y, caches):
        ...
        gradients['dV' + str(L - 1)], gradients['dW' + str(L)], gradients[
            'db' + str(L)] = self.excitation_activation_backward()
        for l in reversed(range(L - 1)):
            current_cache = caches[l]
            gradients['dV' + str(l)], gradients['dW' + str(l + 1)], gradients[
                'db' + str(l + 1)] = self.excitation_activation_backward(self.gradients['dV' + str(l + 1)], current_cache,
